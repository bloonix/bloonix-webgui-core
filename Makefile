	...
	if [ "$(BUILDPKG)" = "0" ] ; then \
		if [ -e /bin/systemctl ] || [ -e /usr/bin/systemctl ] ; then \
			if [ -e /lib/systemd/system/ ] ; then \
				install -m 0644 etc/init/bloonix-webgui.service $(DESTDIR)/lib/systemd/system/; \
			elif [ -e /usr/lib/systemd/system/ ] ; then \
				install -m 0644 etc/init/bloonix-webgui.service $(DESTDIR)/usr/lib/systemd/system/; \
			fi; \
			systemctl daemon-reload; \
		fi; \
		install -d -m 0755 $(INITDIR); \
		install -m 0755 etc/init/bloonix-webgui $(INITDIR)/; \
	fi;
